...
int shim_get_chassis_name(int chassisId, char *chassisName) {
    ...
    strncpy(chassisName, chassis_name[chassisId], strlen(chassis_name[chassisId]));
    return 0;
}
...
int shim_get_snr_name(int chassisId, int snrType, int memberId, char *chassisName) {
    ...
    strncpy(chassisName, snrPtr->name, strlen(snrPtr->name));
    return 0;
}
...
static int read_device(const char *device, int *value) {
    ...
    strncpy(file_name, pch + 1, strlen(device) - pre_dir_len - 1);
